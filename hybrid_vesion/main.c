...
int main(int argc, char *argv[])
{
    ...
    {
        pthread_create(&threads[i], NULL, cast_ray, (void *)i);
    }
...
void *cast_ray(void *id)
{
    int thread_id = (int)id;
    int slice = (end_range - start_range) / N_THREADS;

    // get delimiters
    int start = (thread_id * slice) + start_range;
    int end;
    ...
    // allocate resources
    int local_range = end - start;
    int *thread_local_buffer;
    thread_local_buffer = (int *)(malloc(local_range * 3 * sizeof(int)));
    ...
    for (int r = start; r < end; r++)
    {
        ...
        rt_write_colour(thread_local_buffer, buffer_offset, pixel, number_of_samples);
